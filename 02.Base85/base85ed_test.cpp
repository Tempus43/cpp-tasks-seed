...
TEST(Base85RoundTrip, RandomData) {
    ...
    for (int i = 0; i < 200; ++i) {
        ...
        for (auto& b : original) b = static_cast<uint8_t>(byte_dist(rng));
...
// Каждый возможный однобайтовый вход
...
// Граничные длины вокруг кратных 4
TEST(Base85RoundTrip, BoundaryLengths) {
    for (size_t len : {0, 1, 2, 3, 4, 5, 7, 8, 9, 15, 16, 17, 127, 128, 129}) {
        std::vector<uint8_t> original(len);
        for (size_t i = 0; i < len; ++i) original[i] = static_cast<uint8_t>(i % 251);
        auto decoded = base85::decode(base85::encode(original));
        EXPECT_EQ(decoded, original);
    }
}

// Декодирование строки с неполной последней группой (известные векторы)
